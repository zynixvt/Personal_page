...
export function setupTestHooks() {
  ...
  var gridFn = extractFunction('buildSpatialGrid');
  var renderFn = extractFunction('renderComentario');
  var escapeFn = extractFunction('escapeHtml');
  var diffFn = extractFunction('diffComments');
  ...
  var mockYMStr = [
    ...
    '        warn.textContent = "⚠️ Tenés " + count + " de 100 videos guardados.";',
    '        container.insertBefore(warn, container.firstChild);',
    '      } else {',
    '        existingWarn.textContent = "⚠️ Tenés " + count + " de 100 videos guardados.";',
    '      }',
    ...
    '    opts = opts || {};',
    '    var card = _buildCard(entry);',
    '    card.dataset.videoId = entry.id;',
    '    if (opts.animate !== false) {',
    '      card.classList.add("video-card--enter");',
    '      if (opts.delay) card.style.setProperty("--delay", opts.delay + "s");',
    '    }',
    '    if (opts.prepend) {',
    ...
    '    }',
    '    this._updateCapacityWarning(container);',
    '    return card;',
    '  },',
    ...
    '  getAll: function () {',
    '    try {',
    '      return JSON.parse((globalThis.localStorage || {}).getItem(\'youtube-videos\') || \'[]\');',
    '    } catch (_) { return []; }',
    '  },',
    ...
    '      card.style.setProperty(\'--delay\', Math.min(i, 10) * 0.06 + \'s\');',
    '      card._animated = true;',
    '      card.classList.add(\'video-card--enter\');',
    '    });',
    ...
    '      var warn = document.createElement(\'p\');',
    '      warn.className = \'video-capacity-warn\';',
    '      warn.textContent = \'⚠️ Tenés \' + this.getCount() + \' de 100 videos guardados.\';',
  ...
  var evalCode = [
    ...
    '};',
    '',
    '// === Mock CommentSync and comment render dependencies ===',
    'var listaComentarios = null;',
    'var CommentSync = {',
    '  _enabled: false,',
    '  remove: function () { return { catch: function () {} }; },',
    '  add: function () { return { catch: function () {} }; },',
    '  fetchAll: function () { return Promise.reject(); },',
    ...
    '  addCard: function (entry, container, opts) { return YouTubeManager.addCard(entry, container, opts); },',
    '  removeCard: function (videoId) { return YouTubeManager.removeCard(videoId); },',
    '  animateEntries: function () { return YouTubeManager.animateEntries(); },',
    '  render: function (container) { return YouTubeManager.render(container); },',
    '  updateCapacityWarning: function (container) { return YouTubeManager._updateCapacityWarning(container); },',
    '  resetInitialRender: function () { YouTubeManager._initialRender = false; },',
    '  // Comment system',
    '  renderComentario: function (c) { return renderComentario(c); },',
    '  escapeHtml: function (str) { return escapeHtml(str); },',
    '  diffComments: function (remote, current) { return diffComments(remote, current); }',
    '};',
  ].join('\n');
